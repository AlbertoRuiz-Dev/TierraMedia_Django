...
class InventoryViewSetTests(APITestCase):
    ...
    def setUp(self):
        """
        Prepara los datos iniciales para las pruebas.
        Se crean instancias de personaje, armas y armaduras.
        """
        # Elimina todos los peronsajes
        Character.objects.all().delete()

        # Elimina todas las armas
        Weapon.objects.all().delete()

        # Elimina todas las armaduras
        Armor.objects.all().delete()


        self.character = Character.objects.create(name="Character1")
        self.weapon1 = Weapon.objects.create(name="Sword", damage=50, accuracy=80)
        self.weapon2 = Weapon.objects.create(name="Bow", damage=40, accuracy=90)
        self.armor1 = Armor.objects.create(name="Shield", defense=30)
        self.armor2 = Armor.objects.create(name="Helmet", defense=20)

        self.url_list = reverse('juego:inventory-list')  # URL para la lista de relaciones

    def test_create_inventory(self):
        """
        Verifica que se puede crear un nuevo inventario para un personaje.
        """

        # Enviamos los IDs de las armas y armaduras en la solicitud POST
        data = {
            'character': self.character.id,
            'weapons': [self.weapon1.id, self.weapon2.id],
            'armors': [self.armor1.id, self.armor2.id]
        }

        response = self.client.post(self.url_list, data, format='json')

        # Verificamos que la respuesta sea correcta y que el inventario se haya creado
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(response.data['character'], self.character.id)
        self.assertEqual(len(response.data['weapons']), 2)  # Verifica que se asignaron las armas
        self.assertEqual(len(response.data['armors']), 2)  # Verifica que se asignaron las armaduras

    def test_update_inventory(self):
        """
        Verifica que se puede actualizar un inventario existente.
        """
        # Creamos un inventario inicial
        inventory = Inventory.objects.create(
            character=self.character
        )
        inventory.weapons.add(self.weapon1)
        inventory.armors.add(self.armor1)

        # Nuevas armas y armaduras
        new_weapon = Weapon.objects.create(name="Axe", damage=60, accuracy=70)
        new_armor = Armor.objects.create(name="Plate", defense=40)

        url = f'{self.url_list}{inventory.id}/'  # URL de actualización

        # Enviamos los nuevos datos de armas y armaduras
        data = {
            'character': self.character.id,
            'weapons': [new_weapon.id],
            'armors': [new_armor.id]
        }
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['character'], self.character.id)
        self.assertEqual(len(response.data['weapons']), 1)  # Verifica que solo se asignó la nueva arma
        self.assertEqual(len(response.data['armors']), 1)  # Verifica que solo se asignó la nueva armadura

    def test_partial_update_inventory(self):
        """
        Verifica que se puede realizar una actualización parcial de un inventario.
        """
        # Creamos un inventario inicial
        inventory = Inventory.objects.create(
            character=self.character
        ...
        # Nueva arma para actualizar parcialmente
        new_weapon = Weapon.objects.create(name="Spear", damage=45, accuracy=85)

        url = f'{self.url_list}{inventory.id}/'  # URL de actualización parcial

        # Enviamos solo el ID de la nueva arma
        data = {
            'weapons': [new_weapon.id]
        }
        ...
        self.assertEqual(response.data['character'], self.character.id)
        self.assertEqual(len(response.data['weapons']), 1)  # Verifica que solo se asignó la nueva arma
        self.assertEqual(len(response.data['armors']), 1)  # La armadura no debe cambiar

    def test_delete_inventory(self):
        """
        Verifica que se puede eliminar un inventario existente.
        """
        ...
        url = f'{self.url_list}{inventory.id}/'  # URL de eliminación

        response = self.client.delete(url)

        # Verificamos que la respuesta sea correcta y que el inventario haya sido eliminado
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
        self.assertFalse(Inventory.objects.filter(id=inventory.id).exists())  # Verifica que el inventario haya sido eliminado
